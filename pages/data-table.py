...
layout = html.Div([
    # Dropdown filters
    html.Div([
        html.Label('Filter by Region:', style={'margin-right': '10px', }),  # Add margin between label and dropdown
        dcc.Dropdown(
            id='region',
    ...
    html.Div([
        html.Label('Filter by State:', style={'margin-right': '10px'}),
        dcc.Dropdown(
            id='state',
    ...
    html.Div([
        html.Label('Filter by City:', style={'margin-right': '10px'}),
        dcc.Dropdown(
            id='city',
...
# Callback to update State dropdown based on selected Country
@callback(
...
# Callback to update City dropdown based on selected State and Country
@callback(
    Output('city', 'options'),
    Input('state', 'value'),
    Input('region', 'value')
...
@callback(
    [Output('datatable', 'data'), Output('duplicate-error', 'children')],
    [Input('region', 'value'),
     Input('state', 'value'),
     Input('city', 'value'),
...
def update_table_and_add_entry(region_filter, state_filter, city_filter, n_clicks, order_id, customer_id, product_id, sales, profit):
    ...
    if triggered_by == 'add-entry-btn' and n_clicks > 0:
        ...
        if order_id in filtered_df['Order ID'].values:
            duplicate_order_row = filtered_df[filtered_df['Order ID'] == order_id].iloc[-1]

        if product_id in filtered_df['Product ID'].values:
            duplicate_product_row = filtered_df[filtered_df['Product ID'] == product_id].iloc[-1]

        if customer_id in filtered_df['Customer ID'].values:
            if len(duplicate_order_row) == 0:
                duplicate_customer_row = filtered_df[filtered_df['Customer ID'] == customer_id].sort_values('Order Date').iloc[-1]
            else:
                duplicate_customer_rows = filtered_df[(filtered_df['Customer ID'] == customer_id) &
                                                      (filtered_df['Order Date'] == duplicate_order_row.loc['Order Date'])]
        ...
        new_entry = pd.DataFrame({'Row ID': [len(df) + 1],
                                  'Order ID': [order_id],
                                  ...
                                  'Country': [duplicate_customer_row['Country'] if len(duplicate_customer_row) != 0 else None],
                                  'State': [duplicate_customer_row['State'] if len(duplicate_customer_row) != 0 else None],
                                  'City': [duplicate_customer_row['City'] if len(duplicate_customer_row) != 0 else None],
                                  'Region': [duplicate_customer_row['Region'] if len(duplicate_customer_row) != 0 else None],
                                  'Postal Code': [duplicate_customer_row['Postal Code'] if len(duplicate_customer_row) != 0 else None],
        ...
        duplicate = filtered_df[(filtered_df['Customer ID'] == customer_id) &
                                (filtered_df['Product ID'] == product_id) &
                                (filtered_df['Order ID'] == order_id)]

        if len(duplicate) != 0:
            return filtered_df.to_dict('records'), "Duplicate entry! Please try again with different values."
        else:
            # Concatenate new entry with existing data
            new_entries.append(new_entry)
            for entry in new_entries:
                filtered_df = pd.concat([filtered_df, entry], ignore_index=True)

            return filtered_df.to_dict('records'), ""  # Clear the error message
